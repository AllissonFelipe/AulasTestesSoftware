...
def potencia():
    resultado.set(float(numero1.get()) ** float(numero2.get()))
def raiz():
    resultado.set(float(math.sqrt(float(numero1.get()))))
...
tk.Button(frame2, text="Potência", width=15, height=10, command=potencia).grid(row=2, column=0, pady=5, padx=5)
tk.Button(frame2, text="Raiz", width=15, height=10, command=raiz).grid(row=2, column=1, pady=5, padx=5)
